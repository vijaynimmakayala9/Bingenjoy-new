$(function () {
  $.ajax({
    ...
    success: function (t) {
      $("#data").html(
        '<div className="row wow fadeInLeft animated my-3 justify-content-center" id="locations"></div>'
      ),
        $();
      let a = JSON.parse(t);
      for (let e = 0; e < a.res.length; e++)
        $("#locations").append(`
            ...
            <div className="card shadow" style="border-radius: 16px;">
            <div className="card-head">
            <img className="img-fluid" style="border-top-right-radius: 16px;
          ...
          a.date
            ...
            <a href="selectscreens/${
              a.res[e].location_id
            }" name="submit" value="submit" className="mb-3 theme-btn btn-style-one" id="bookscreen">
            <span className="btn-title">View Screens&nbsp;<i className="fa fa-paper-plane" aria-hidden="true"></i></span>
        </a>
            </div>
        </div> 
            </div>
        ...
        success: function (t) {
          $(`#itemd${a}`).hasClass("alert-primary2")
            ? ($(`#itemd${a}`).removeClass("alert-primary2"),
              $(`#status${a}`).html(
                ' Add To Booking &nbsp; <i className="fa fa-plus"></i>'
              ),
